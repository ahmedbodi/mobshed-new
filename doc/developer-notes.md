...
- [Development guidelines](#development-guidelines)
    - [General Mobshed Core](#general-mobshed-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Mobshed Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play mobsheds" on the test network, if you
...
Mobshed Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/mobshed-project/mobshed/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_mobshed
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_mobshed --log_level=test_suite
$ valgrind -v --leak-check=full src/mobshedd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_mobshed.coverage/index.html`.
```
...
To profile a running mobshedd process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep mobshedd` -- sleep 60
```
...
Mobshed Core can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as Mobshed Core, where everyone uses
...
pay attention to for reviewers of Mobshed Core code.

General Mobshed Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Mobshed Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
Some of these are maintained by active developers of Mobshed Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against Mobshed Core so that they can be integrated
...
file descriptors it will cause problems with Mobshed's `select()` loop, because
...
would be to revert the upstream fix before applying the updates to Mobshed's
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `mobshed-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `mobshed-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Mobshed Core, the wallet was always
