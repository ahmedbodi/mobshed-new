Mobshed Core version *0.15.0* is now available from:

  <https://download.mobshed.org/mobshed-0.15.0/>
...
installer (on Windows) or just copy over `/Applications/Mobshed-Qt` (on Mac)
or `mobshedd`/`mobshed-qt` (on Linux).
...
Mobshed Core is extensively tested on multiple operating systems using
...
- The `estimatesmartfee` RPC interface has been changed (See [PR 10707](https://github.com/bitcoin/bitcoin/pull/10707)):
    ...
    - If Mobshed Core has not been running for long enough and has not seen enough blocks or transactions to produce an accurate fee estimation, an error will be returned (previously a value of -1 was used to indicate an error, which could be confused for a feerate).
...
Mobshed Core now supports loading multiple, separate wallets (See [PR 8694](https://github.com/bitcoin/bitcoin/pull/8694), [PR 10849](https://github.com/bitcoin/bitcoin/pull/10849)). The wallets are completely separated, with individual balances, keys and received transactions.

Multi-wallet is enabled by using more than one `-wallet` argument when starting Mobshed, either on the command line or in the Mobshed config file.

**In Mobshed-Qt, only the first wallet will be displayed and accessible for creating and signing transactions.** GUI selectable multiple wallets will be supported in a future version. However, even in 0.15 other loaded wallets will remain synchronized to the node's current tip in the background. This can be useful if running a pruned node, since loading a wallet where the most recent sync is beyond the pruned height results in having to download and revalidate the whole blockchain. Continuing to synchronize all wallets in the background avoids this problem.

Mobshed Core 0.15.0 contains the following changes to the RPC interface and `mobshed-cli` for multi-wallet:

* When running Mobshed Core with a single wallet, there are **no** changes to the RPC interface or `mobshed-cli`. All RPC calls and `mobshed-cli` commands continue to work as before.
* When running Mobshed Core with multi-wallet, all *node-level* RPC methods continue to work as before. HTTP RPC requests should be send to the normal `<RPC IP address>:<RPC port>/` endpoint, and `mobshed-cli` commands should be run as before. A *node-level* RPC method is any method which does not require access to the wallet.
* When running Mobshed Core with multi-wallet, *wallet-level* RPC methods must specify the wallet for which they're intended in every request. HTTP RPC requests should be send to the `<RPC IP address>:<RPC port>/wallet/<wallet name>/` endpoint, for example `127.0.0.1:9332/wallet/wallet1.dat/`. `mobshed-cli` commands should be run with a `-rpcwallet` option, for example `mobshed-cli -rpcwallet=wallet1.dat getbalance`.
...
- When using Mobshed Core in multi-wallet mode, RPC requests for wallet methods must specify
